...
impl Gear {
    fn new(coordinate: Coordinate) -> Self {
        Gear {
            coordinates: (coordinate.adjacent_coordinates().into_iter().collect()),
        }
    }
...
fn main() {
    ...
    for (y, line) in my_str.enumerate() {
        for (x, char) in line.chars().enumerate() {
            ...
            if char == '*' {
                gears.push(Gear::new(Coordinate::new(x, y)));
            }
        }
    }
    let mut sum = 0;
    for gear in gears {
        ...
        for matrix_number in m_numbers.iter() {
            if !matrix_number
                .get_coordinates()
                .intersection(gear.get_coordinates())
                .collect::<Vec<&Coordinate>>()
                .is_empty()
            {
                product *= matrix_number.get_value();
                counter += 1;
            }
        }
        if counter == 2 {
